...
export const EmailTemplateEditorModal: React.FC<EmailTemplateEditorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[96vh] flex flex-col overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-xs font-mono text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition"
              />
            </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {['name', 'firstName', 'company', 'designation', 'email'].map((v) => (
                    <button
                      ...
                      className="px-1.5 py-0.5 bg-slate-800 hover:bg-slate-700 text-emerald-400 text-[10px] font-mono rounded border border-slate-700/80 transition"
          ...
          <div className="lg:col-span-5 flex flex-col space-y-3">
            ...
            <div
              className={`flex-1 bg-slate-950 border border-slate-800 rounded-2xl flex flex-col overflow-hidden shadow-inner ${
                previewDevice === 'MOBILE' ? 'max-w-[340px] mx-auto' : 'w-full'
              }`}
            >
              {/* Email Envelope Header */}
              <div className="p-3.5 bg-slate-900 border-b border-slate-800/80 space-y-1.5 text-xs">
                <div className="flex items-center justify-between text-[11px] text-slate-400">
                  <span>To: <strong className="text-slate-200">Tariq Farooq</strong> &lt;tariq@albait-tours.com&gt;</span>
                  <span className="text-[10px] font-mono text-emerald-400">Umrah360 Outbound</span>
                </div>
                <div className="text-[11px] text-slate-400">
                  <span>From: <span className="text-slate-300 font-medium">Umrah360 Team</span> &lt;amaavigo@gmail.com&gt;</span>
                </div>
                <div className="pt-1 border-t border-slate-800 text-xs font-semibold text-emerald-300">
                  <span className="text-slate-400 font-normal mr-1">Subject:</span>
                  {renderPreviewWithSampleLead(subject) || 'No Subject'}
                </div>
              </div>

              {/* Rendered Email Body */}
              <div className="flex-1 p-4 bg-slate-950 overflow-y-auto text-slate-200 text-xs leading-relaxed font-sans space-y-2">
                {formatMode === 'PLAIN' ? (
                  ...
                  <div
                    className="email-rendered-content prose prose-invert max-w-none text-xs leading-relaxed"
                ...
                <div className="p-3 bg-slate-900 border-t border-slate-800 space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {attachments.map((att) => (
                      <div
                        key={att.id}
                        className="px-2.5 py-1 bg-slate-950 border border-slate-800 rounded-lg text-[10px] text-slate-200 flex items-center gap-1.5"
                      >
                        {getFileIcon(att.name, att.type)}
                        <span className="max-w-[120px] truncate font-medium">{att.name}</span>
                        <span className="text-slate-500 font-mono">({formatFileSize(att.size)})</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="text-[10px] text-slate-500 text-center">
              * Variables like <code className="text-emerald-400 font-mono">{"{{name}}"}</code> & <code className="text-emerald-400 font-mono">{"{{company}}"}</code> are automatically substituted per lead.
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-slate-800 bg-slate-950 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              onClick={onClose}
              disabled={isSaving}
              className="px-4 py-2 text-xs text-slate-400 hover:text-slate-200 transition font-medium"
            >
              Cancel
            </button>
            <button
              ...
              disabled={isSaving}
              className="inline-flex items-center gap-1.5 px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-xl text-xs font-semibold shadow-lg shadow-emerald-900/20 transition"
            >
              {isSaving ? (
                <>
