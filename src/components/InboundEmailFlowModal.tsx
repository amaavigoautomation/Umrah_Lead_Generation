...
export const InboundEmailFlowModal: React.FC<InboundEmailFlowModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md overflow-y-auto font-sans">
      <div className="bg-white border border-slate-200 rounded-2xl max-w-4xl w-full max-h-[92vh] flex flex-col shadow-2xl overflow-hidden my-auto">
        {/* Header */}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="text-lg font-bold text-slate-900">Live Inbound Email Pipeline</h3>
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-mono font-bold bg-orange-50 text-orange-700 border border-orange-200">
                  {INBOUND_MAILBOX}
                </span>
                <span className="flex items-center space-x-1 text-[11px] text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full border border-orange-200">
                  <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse"></span>
                  <span>Active Listener</span>
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">
                Simulates or receives real inbound emails sent to <strong className="text-slate-800">{INBOUND_MAILBOX}</strong> and executes the complete 8-step inbound mail flow.
              </p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-xs">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className={`px-3 py-1 rounded-md transition font-medium ${
                    !isCustom ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-600 hover:text-slate-900'
                  }`}
                >
            ...
            {!isCustom ? (
                ...
                {PRESET_INBOUND_EMAILS.map((preset) => (
                  <button
                    ...
                    className={`p-3.5 rounded-xl border text-left transition flex flex-col justify-between ${
                      selectedPresetId === preset.id
                        ? 'bg-orange-50/60 border-orange-300 shadow-sm ring-1 ring-orange-300'
                  ...
                  >
                    <div>
                      ...
                      <h4 className="font-bold text-slate-900 text-xs mt-1">{preset.label}</h4>
                      <p className="text-slate-500 text-[11px] mt-1 line-clamp-2">{preset.description}</p>
                    </div>
                    <div className="text-[10px] font-mono text-slate-400 mt-2 pt-2 border-t border-slate-100 truncate">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="text-[11px] font-medium text-slate-600">Sender Email</label>
                    <input
                      type="email"
                      value={customFrom}
                      onChange={(e) => setCustomFrom(e.target.value)}
                      className="mt-1 w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-800 focus:outline-none focus:border-orange-500"
                    />
                  </div>
                  <div>
                    <label className="text-[11px] font-medium text-slate-600">Sender Name</label>
                    <input
                      type="text"
                      value={customName}
                  ...
                  <div>
                    <label className="text-[11px] font-medium text-slate-600">Agency / Company</label>
                    <input
                      type="text"
                      value={customCompany}
                ...
                <div>
                  <label className="text-[11px] font-medium text-slate-600">Email Subject</label>
                  <input
                    type="text"
                    value={customSubject}
                ...
                <div>
                  <label className="text-[11px] font-medium text-slate-600">Email Body</label>
                  <textarea
                    rows={4}
                    value={customBody}
                    onChange={(e) => setCustomBody(e.target.value)}
                    className="mt-1 w-full bg-white border border-slate-200 rounded-lg p-3 text-xs text-slate-800 focus:outline-none focus:border-orange-500 font-mono"
                  />
                </div>
              </div>
            )}
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2">
            <div className="flex items-center justify-between border-b border-slate-200 pb-2">
              <span className="text-[11px] font-bold text-slate-800">Incoming Message Inspector</span>
              <span className="text-[10px] font-mono text-slate-500">To: {INBOUND_MAILBOX}</span>
            </div>
            ...
            <div className="space-y-1 text-slate-700">
              ...
              <div>
                <span className="text-slate-500">Subject:</span> <strong className="text-slate-900">{activePayload.subject}</strong>
              </div>
              <div className="pt-2 text-slate-800 whitespace-pre-wrap font-sans bg-white p-3 rounded-lg border border-slate-200">
                {activePayload.body}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-semibold transition flex items-center justify-center space-x-2 shadow-md disabled:opacity-50"
            >
              {isExecuting ? (
                <>
